...
def get_gemini_response(question, prompt):
    ...
    return response.text  # Return the raw response for chatbot
...
# Chatbot Interface
with col1:
    st.header("Chat with Herbal Bot")

    # Use session state to store user input and avoid duplicate widget keys
    st.session_state.chat_input = st.text_input("Ask me anything about plants:", value=st.session_state.chat_input)

    if st.button("Send"):
        if st.session_state.chat_input:
            # Chatbot QnA response
            bot_response = get_gemini_response(st.session_state.chat_input, prompt)
            st.session_state.chat_history.append({"user": st.session_state.chat_input, "bot": bot_response})
    ...
    if st.session_state.plant_name:
        ...
        if st.button("Save Location"):
            ...
            if record_exists and record_exists[0][0] > 0:
                update_query = f"""
                UPDATE user_loc 
                SET presence = {presence_value}, Last_Confirmed = CURRENT_DATE
                WHERE Scientific_Name = '{st.session_state.plant_name}' 
                AND latitude = {latitude} 
                AND longitude = {longitude}
                """
            ...
            # If the record does not exist, perform an insert
                ...
                insert_query = f"""
                INSERT INTO user_loc (Scientific_Name, latitude, longitude, presence, Date_found, Last_Confirmed) 
                VALUES ('{st.session_state.plant_name}', {latitude}, {longitude}, {presence_value}, CURRENT_DATE, CURRENT_DATE)
                """
                try:
