...
# Define your prompt for the chatbot
chat_prompt = """
You are an expert in answering questions related to medicinal plants. You have access to a database called 'herbalhug' that contains the following columns: ID, Plant_Name, Scientific_Name, Common_Name, Family_Name, Uses, and Location.
Generate SQL queries based on user input to find relevant data in the database. If the information is not in the database, answer the user using external knowledge.
"""

# Chatbot function
def chatbot_response(question):
    # Try to match a SQL query for the database
    try:
        sql_query = get_gemini_response(question, [chat_prompt])
        ...
        # If data found in the database, return it as the response
        if result:
            return f"Here's what I found: {result}"
        else:
            raise ValueError("No data found in the database for this query.")
    
    except Exception as e:
        # If no SQL data or error, use external knowledge (Google GenAI)
        external_response = get_gemini_response(question, ["Answer this question using external sources."])
...
with col1:
    st.header("Search")
    question = st.text_input("Ask a question about medicinal plants:", key="input")
...
if location is not None:
    ...
    if st.session_state.plant_name:
        ...
        # New feature: Add remark
        remark = st.text_input("Add any remarks about the plant at this location:")

        if st.button("Save Location and Remark"):
            ...
            if record_exists and record_exists[0][0] > 0:
                update_query = f"""
                UPDATE user_loc 
                SET presence = {presence_value}, Last_Confirmed = CURRENT_DATE, remarks = '{remark}'
                WHERE Scientific_Name = '{st.session_state.plant_name}' 
                AND latitude = {latitude} 
                AND longitude = {longitude}
                """
                try:
                    ...
                    st.success(f'Location, plant presence, and remark updated: Latitude - {latitude}, Longitude - {longitude}')
                except Exception as e:
                    st.error(f'Failed to update location, plant presence, and remark: {e}')
            # If the record does not exist, perform an insert
                ...
                try:
                    ...
                    st.success(f'New location, plant presence, and remark saved: Latitude - {latitude}, Longitude - {longitude}')
                except Exception as e:
                    st.error(f'Failed to save new location, plant presence, and remark: {e}')
    else:
